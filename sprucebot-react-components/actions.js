...
export function fetchLocationUserAndRole(locationId = parse(window.location.search).locationId, userId = parse(window.location.search).userId, allowCache = true) {
		...
		try {
			...
			if (allowCache) {
				...
				if (state.skill.location && state.skill.user && state.skill.role) {
					return dispatch(receiveLocationUserAndRole(state.location, state.user, state.role));
				}
			}

			//let the world know we are starting
			dispatch(requestLocationUserAndRole(locationId, userId));
			const { data: { location, user, role } } = await get(`/locations/${locationId}/users/${userId}.json`);
...
export function fetchGuests(allowCache = true) {
	...
	return async function (dispatch, getState) {
		...
		try {
			...
			return dispatch(receiveGuests(guests.map(guest => {
				return Object.assign({ online: guest.status === 'online', visits: guest.visits }, guest.User);
			})));
		} catch (err) {
			console.log(err);
			alert(err.message);
