...
api.getData = function (cb, query) {
    ...
    this.get('/data', function (err, data) {
...
api.patchDataAsObject = function (dataObject, cb, query) {
    ...
    api.getData(function (err, data) {
        ...
        if (remaining.length > 0) {
            cb(new Error('Cannot patch data for missing keys (' + remaining.join(', ') + '). Make sure you set them in exports.defaultData at the top of controller.js'));
            errored = true;
        }
...
api.setupDefaultDataForLocation = function (locationId, defaults) {
    ...
    this.getData(function (err, data) {
        ...
        keys.forEach(function (key) {
            ...
            this.postData(key, defaults[key], {
                locationId: locationId
...
api.setupDefaultData = function (defaults) {
    ...
    this.locations(function (err, locations) {
